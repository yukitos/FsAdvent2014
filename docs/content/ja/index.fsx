...
> 2015/04/22 追記
> 
> `build.cmd AddLangDocs` コマンドに不具合があり、各言語用のテンプレートファイルが
> 正しく適用されていないことがわかりました。
> bleisさんありがとうございます。
> 
> 2015/4/22現在の最新版では修正されていますので、
> これから使い始める場合には問題無いはずです。
> 
> 既に使い始めている場合には`docs/tools/generate.fsx`を以下と同じように
> 修正すればいいはずです：
>
...
> 
> Chocolateyがどのくらい簡単かはまだ知らないんですが、
> 言語別のドキュメントを簡単に用意できるコマンドを
> [F# Project Scaffold][fsscaffold]に追加しました。
> 
...
> 2014/12/6 追記
> 
> 下記の`GenerateHelpDebug`ターゲットは[F# Project Scaffold][fsscaffold]に取り込んでもらえたので
> `build.fsx`を編集する必要は無くなっています。
